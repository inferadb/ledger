...
```
gRPC Services (Admin, Read, Write, Organization, Vault, User, App, Token, Events, Health, Discovery, Raft)
 ↓
...
- `TokenError`: 15-variant snafu error enum (Expired, InvalidSignature, Revoked, InvalidAudience, NoActiveSigningKey, RefreshTokenReuse, TokenVersionMismatch, etc.)
...
- `EventAction`: 34 variants (23 system, 11 org) — includes 6 token-related variants: `SigningKeyCreated`, `SigningKeyRotated`, `SigningKeyRevoked`, `TokenCreated`, `TokenRevoked`, `TokenRefreshed` with exhaustive `scope()`, `event_type()`, `as_str()` — adding a variant without updating all three is a compile error. System variants include `UserErased` and `UsersMigrated` for crypto-shredding and flat-to-regional migration.
...
- `store_org_key()`, `get_org_key()`: Per-organization encryption key CRUD for crypto-shredding
...
- OrgKey keys: `org_key(OrganizationId)` (format: `"_key:org:{org_id}"`), `parse_org_key()`, `ORG_KEY_PREFIX` (`"_key:org:"`)
- Assertion name keys: `assertion_name_key(organization, app, assertion)` (format: `"_sys:assertion_name:{org_id}:{app_id}:{assertion_id}"`), `assertion_name_prefix(organization, app)`, `assertion_name_org_prefix(organization)`
- **Insights**: Dedicated key scheme for system metadata. The `_tmp:` prefix convention separates ephemeral onboarding data from permanent `_sys:` system data, `_idx:` index data, and `_key:` encryption keys — makes GC prefix scans unambiguous. The `_key:` prefix isolates encryption material from operational data. Signing key scope index points to the current active key's kid (replaced atomically on rotation). Refresh token indexes use hex-encoded family/hash bytes. Poisoned family key is an O(1) existence check for reuse detection.
...
- `OrgKey` struct: `organization: OrganizationId`, `key: [u8; 32]`, `created_at: DateTime<Utc>` — per-organization AES-256-GCM encryption key for crypto-shredding (destroying key makes org data unreadable in Raft logs)
...
- Service traits: ReadService, WriteService, AdminService, EventsService, TokenService, HealthService, SystemDiscoveryService, RaftService, OrganizationService, VaultService
...
- `load_key()`, `evict_key()`, `get_cached_key()`, `has_cached_key()`: ArcSwap copy-on-write cache management
- `JwtError`: 7-variant snafu error (Signing, Decoding, KeyDecryption, KeyEncryption, Token, OrganizationNotFound, StateLookup)
...
- `services/organization.rs`: OrganizationService gRPC implementation — `get_organization_team()` (reads team profile, checks membership), `add_team_member()` (proposes `AddTeamMember` to REGIONAL Raft), `remove_team_member()` (proposes `RemoveTeamMember` to REGIONAL Raft). Uses `propose_regional_org_encrypted()` for org-scoped PII encryption.
...
- `services/service_infra.rs`: `ServiceContext` — `propose_regional_org_encrypted()` reads OrgKey from state, encrypts SystemRequest with per-org key, wraps in `EncryptedOrgSystem`. Existing `propose_regional_encrypted()` renamed to use `EncryptedUserSystem` wrapper.
...
- `entry_crypto.rs`: AES-256-GCM encryption for Raft log entries. `EncryptedUserSystemRequest` (per-user subject key encryption), `EncryptedOrgSystemRequest` (per-org OrgKey encryption for crypto-shredding). Generic `seal()`/`unseal()` helpers. `OnboardingPii` sealed during saga step 1. Organization ID used as AAD to prevent cross-org entry substitution.
...
- `BootstrappedNode`: Handle to running node (server, Raft, 13 tracked background job handles, `saga_orchestrator_handle: Option<SagaOrchestratorHandle>`)
- Background job handles: `gc_handle`, `compactor_handle`, `recovery_handle`, `learner_refresh_handle`, `resource_metrics_handle`, `backup_handle` (optional), `events_gc_handle` (optional), `saga_handle`, `orphan_cleanup_handle`, `integrity_scrub_handle`, `snapshot_demotion_handle` (optional), `token_maintenance_handle`, `post_erasure_compaction_handle`
...
- **Per-organization crypto-shredding**: OrgKey (AES-256-GCM per-org key) seals org-scoped PII in Raft entries. `EncryptedOrgSystem` wraps SystemRequests encrypted with OrgKey. Organization purge destroys OrgKey, making all org data in Raft logs unreadable. `PostErasureCompactionJob` triggers proactive snapshots to evict plaintext from log retention window. Retry-resilient purge with exponential backoff.
