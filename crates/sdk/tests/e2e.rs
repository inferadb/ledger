...
async fn test_write_read_cycle() {
    ...
    let read_result =
        client.read(ns_id, Some(vault), "user:alice").await.expect("read should succeed");
...
async fn test_multiple_writes_reads() {
    ...
    for i in 0..5 {
        ...
        let result = client.read(ns_id, Some(vault), &key).await.expect("read should succeed");
...
async fn test_batch_read() {
    ...
    let results =
        client.batch_read(ns_id, Some(vault), keys).await.expect("batch read should succeed");
...
async fn test_write_replication_to_followers() {
    ...
    for (i, ep) in non_leaders.iter().enumerate() {
        ...
        let read_result = follower_client
            .read(ns_id, Some(vault), "repl:key")
            .await
...
async fn test_three_node_write_replication() {
    ...
    for (i, ep) in endpoints.iter().enumerate() {
        ...
        let result =
            reader.read(ns_id, Some(vault), "replicated:key").await.expect("read from node");
...
async fn test_multiple_client_sessions() {
    ...
    // Read back both keys
    let value1 = client2.read(ns_id, Some(vault), "session:key1").await.expect("read first");
    let value2 = client2.read(ns_id, Some(vault), "session:key2").await.expect("read second");
...
async fn test_data_persistence_across_sessions() {
    ...
    // Second client session — read previously written data
    {
        ...
        for i in 0..5 {
            ...
            let read_result = client.read(ns_id, Some(vault), &key).await.expect("read");
        ...
        let read_result = client.read(ns_id, Some(vault), key).await.expect("read new write");
