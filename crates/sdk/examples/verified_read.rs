...
use inferadb_ledger_sdk::{
    ClientConfig, LedgerClient, Operation, Region, Result, ServerSource, VerifyOpts,
};
...
async fn main() -> Result<()> {
    ...
    // -------------------------------------------------------------------------
    let org = client.create_organization("verified_example", Region::US_EAST_VA).await?;
    ...
    let result =
        client.verified_read(organization, Some(vault), test_key, VerifyOpts::new()).await?;
    ...
    let historical = client.verified_read(organization, Some(vault), test_key, opts).await?;
