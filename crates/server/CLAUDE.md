...
7. **Bootstrap starts only the system group.** `bootstrap_node()` brings up the `GLOBAL` region's data-region group at `OrganizationId(0)` (the system group's host until the skeleton `SystemGroup` fills its own fields). Data regions and per-organization groups are created through cluster consensus — `CreateDataRegion` flows through the system group's apply pipeline, and `CreateOrganization` fans out through the multi-tier orchestration (system apply → region placement → per-voter `start_organization_group`). Do not eagerly bootstrap data regions or organization groups from `main.rs` — that path is reserved for the `init` CLI subcommand acting on cluster consensus.
