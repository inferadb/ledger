...
use inferadb_ledger_types::{
    CredentialData, CredentialType, EmailVerifyTokenId, InvitationStatus, InviteId, NodeId,
    Operation, OrganizationId, OrganizationInvitation, OrganizationSlug, PasskeyCredential,
    PendingTotpChallenge, Region, SetCondition, SigningKeyScope, TokenVersion, UserCredential,
    UserCredentialId, UserEmailId, UserId, UserRole, UserSlug, UserStatus, VaultId, VaultSlug,
    decode, encode, hash_eq,
};
...
impl<B: StorageBackend> SystemOrganizationService<B> {
    ...
    /// 6. Delete all user credentials (passkeys, TOTP, recovery codes) and type indices.
    /// 7. Delete all pending TOTP challenges.
    /// 8. Delete per-subject encryption key.
    /// 9. Delete the User record (contains plaintext name — PII).
    /// 10. Write erasure audit record (insert-if-absent for idempotency).
    ///
    ...
    pub fn erase_user(
        ...
        // Step 3: Revoke all refresh token families and bump token version.
        // Version bump is the backstop that invalidates all outstanding JWTs.
        ...
        // Uses delete_all_user_credentials which bypasses the last-credential guard.
        if let Err(e) = self.delete_all_user_credentials(user_id) {
            tracing::warn!(
                user_id = user_id.value(),
                error = %e,
                "Failed to delete user credentials during erasure"
            );
        }
        ...
        if let Err(e) = self.delete_all_totp_challenges(user_id) {
            tracing::warn!(
                user_id = user_id.value(),
                error = %e,
                "Failed to delete TOTP challenges during erasure"
            );
        }
        ...
        // Step 9: Delete the User record (contains plaintext name — PII).
        ...
        // Step 10: Write erasure audit record (insert-if-absent for idempotency).
...
mod tests {
    ...
    fn test_erase_user_end_to_end() {
        // Full lifecycle: create → store key → register email → add credentials → erase → verify
        // all gone
        let svc = create_test_service();
        let user_id = UserId::new(42);
        let now = Utc::now();
        ...
        // Create a credential and TOTP challenge
        let cred = svc
            ...
            .unwrap();
        let challenge_nonce = [0xE2; 32];
        svc.create_totp_challenge(&make_challenge(user_id, UserSlug::new(10042), challenge_nonce))
            .unwrap();
        ...
        assert!(svc.get_totp_challenge(user_id, &challenge_nonce).unwrap().is_some());
        ...
        assert!(svc.get_totp_challenge(user_id, &challenge_nonce).unwrap().is_none());
    ...
    fn test_erase_user_deletes_email_records() {
        ...
        let email_id = inferadb_ledger_types::UserEmailId::new(700);
        let email2_id = inferadb_ledger_types::UserEmailId::new(701);

        // Create user directory
        let entry = make_directory_entry(70, 10070, Region::US_EAST_VA);
        svc.register_user_directory(&entry).unwrap();

        // Create flat User record (needed for email ops)
        let user = make_test_user(70, 10070, Region::US_EAST_VA);
        write_flat_user(&svc, &user);

        // Create two email records and index
        let email1 = make_test_user_email(700, 70, "primary@example.com");
        let email2 = make_test_user_email(701, 70, "secondary@example.com");
        ...
        assert!(svc.get_user_email(email2_id).unwrap().is_some());
        assert_eq!(svc.get_user_email_ids(user_id).unwrap().len(), 2);

        // Erase
        svc.erase_user(user_id, Region::US_EAST_VA, Utc::now()).unwrap();

        // Verify email records are deleted
    ...
    fn test_erase_user_deletes_credentials() {
        ...
        // Create user directory
        let entry = make_directory_entry(91, 10091, Region::US_EAST_VA);
        svc.register_user_directory(&entry).unwrap();

        // Create credentials: passkey + TOTP
        let pk = svc
            .create_user_credential(
                user_id,
                CredentialType::Passkey,
        ...
        // Verify credentials exist pre-erasure
        assert!(svc.get_user_credential(user_id, pk.id).unwrap().is_some());
        assert!(svc.get_user_credential(user_id, totp.id).unwrap().is_some());
        assert_eq!(svc.list_user_credentials(user_id, None).unwrap().len(), 2);

        // Erase
        svc.erase_user(user_id, Region::US_EAST_VA, now).unwrap();

        // Verify credentials are deleted (primary records)
        assert!(svc.get_user_credential(user_id, pk.id).unwrap().is_none());
        assert!(svc.get_user_credential(user_id, totp.id).unwrap().is_none());
        assert!(svc.list_user_credentials(user_id, None).unwrap().is_empty());

        // Verify type index entries are also deleted
        let type_idx_prefix = SystemKeys::user_credential_type_index_prefix(user_id);
        let idx_entries =
            svc.state.list_entities(SYSTEM_VAULT_ID, Some(&type_idx_prefix), None, 100).unwrap();
        assert!(idx_entries.is_empty(), "type index entries should be cleaned up during erasure");
    ...
    fn test_erase_user_deletes_totp_challenges() {
        ...
        // Create user directory
        let entry = make_directory_entry(92, 10092, Region::US_EAST_VA);
        svc.register_user_directory(&entry).unwrap();

        // Create TOTP challenges
        svc.create_totp_challenge(&make_challenge(user_id, UserSlug::new(10092), nonce1)).unwrap();
        svc.create_totp_challenge(&make_challenge(user_id, UserSlug::new(10092), nonce2)).unwrap();

        // Verify challenges exist pre-erasure
        assert!(svc.get_totp_challenge(user_id, &nonce1).unwrap().is_some());
        assert!(svc.get_totp_challenge(user_id, &nonce2).unwrap().is_some());

        // Erase
        svc.erase_user(user_id, Region::US_EAST_VA, now).unwrap();

        // Verify challenges are deleted
        assert!(svc.get_totp_challenge(user_id, &nonce1).unwrap().is_none());
        assert!(svc.get_totp_challenge(user_id, &nonce2).unwrap().is_none());
    ...
    fn test_erase_user_credentials_other_users_unaffected() {
        ...
        let entry1 = make_directory_entry(93, 10093, Region::US_EAST_VA);
        svc.register_user_directory(&entry1).unwrap();
        let entry2 = make_directory_entry(94, 10094, Region::US_EAST_VA);
        svc.register_user_directory(&entry2).unwrap();
        ...
        // Create TOTP challenges for both users
        ...
        // User 93's data is gone
        assert!(svc.list_user_credentials(user_id, None).unwrap().is_empty());
        assert!(svc.get_totp_challenge(user_id, &[0xA0; 32]).unwrap().is_none());

        // User 94's data is untouched
    ...
    use inferadb_ledger_types::{
        ...
        RecoveryCodeCredential, TotpAlgorithm, TotpCredential, UserCredentialId,
    };
    ...
    fn make_test_invitation(org: OrganizationId, invite: InviteId) -> OrganizationInvitation {
        use inferadb_ledger_types::{InviteSlug, OrganizationMemberRole, TeamId};
