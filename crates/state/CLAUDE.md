...
11. **Storage layout is per-organization under each region.** Each `(region, organization_id)` owns its own directory at `{data_dir}/{region}/{organization_id}/` holding state.db, raft.db, blocks.db, events.db. The data-region group uses `OrganizationId(0)` (the system organization); per-organization groups use their assigned `OrganizationId`. The `global/0/` path hosts the cluster control plane. Introducing a new Raft group requires a new `(region, organization_id)` directory — never co-mingle two groups' state DBs, and never flatten the directory layout (e.g., `{data_dir}/{region}/`) without reworking every apply / checkpoint / backup path in lockstep.
