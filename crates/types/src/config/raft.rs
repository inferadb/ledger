...
/// Default flush interval in milliseconds for batched cross-group Raft messages.
const fn default_batch_raft_flush_interval_ms() -> u64 {
    1
}

/// Default maximum messages per batch before forced flush.
const fn default_batch_raft_max_batch_size() -> usize {
    64
}

/// Configuration for batched cross-group Raft message delivery.
///
/// When enabled, outgoing `AppendEntries` messages destined for the same
/// peer node across different Raft groups are coalesced into a single
/// `BatchSend` RPC. This reduces per-message gRPC overhead in multi-group
/// deployments.
///
/// Disabled by default -- opt-in optimization.
///
/// # Validation Rules
///
...
/// let config = BatchRaftConfig::builder()
///     .enabled(true)
///     .flush_interval_ms(2)
///     .max_batch_size(128)
///     .build()
///     .expect("valid batch raft config");
/// ```
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub struct BatchRaftConfig {
    /// Whether cross-group message batching is enabled.
    #[serde(default)]
    pub enabled: bool,
    /// Flush interval in milliseconds.
    ///
    /// Pending messages are flushed after this interval even if the batch
    /// is not full. Must be > 0.
    #[serde(default = "default_batch_raft_flush_interval_ms")]
    pub flush_interval_ms: u64,
    /// Maximum messages per batch before a forced flush.
    ///
    /// Must be > 0.
    #[serde(default = "default_batch_raft_max_batch_size")]
    pub max_batch_size: usize,
}

#[bon::bon]
impl BatchRaftConfig {
    /// Creates a new batched Raft message configuration with validation.
    ///
    /// # Errors
    ///
    /// Returns [`ConfigError::Validation`] if:
    ...
    pub fn new(
        ...
        #[builder(default = default_batch_raft_flush_interval_ms())] flush_interval_ms: u64,
        #[builder(default = default_batch_raft_max_batch_size())] max_batch_size: usize,
    ) -> Result<Self, ConfigError> {
        let config = Self { enabled, flush_interval_ms, max_batch_size };
        config.validate()?;
        Ok(config)
    }
}
...
impl BatchRaftConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        if self.flush_interval_ms == 0 {
            return Err(ConfigError::Validation {
                message: "flush_interval_ms must be > 0".to_string(),
            });
        }
        if self.max_batch_size == 0 {
            return Err(ConfigError::Validation {
...
impl Default for BatchRaftConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            flush_interval_ms: default_batch_raft_flush_interval_ms(),
            max_batch_size: default_batch_raft_max_batch_size(),
        }
    }
}
...
mod tests {
    ...
    // =========================================================================
    // BatchRaftConfig tests
    // =========================================================================

    #[test]
    fn batch_raft_config_default_is_valid() {
    ...
    #[test]
    fn batch_raft_config_builder_defaults() {
        let config = BatchRaftConfig::builder().build().unwrap();
        assert_eq!(config, BatchRaftConfig::default());
    }

    #[test]
    fn batch_raft_config_builder_enabled() {
        let config = BatchRaftConfig::builder().enabled(true).build().unwrap();
        assert!(config.enabled);
    }

    #[test]
    fn batch_raft_config_zero_flush_interval_fails() {
        let result = BatchRaftConfig::builder().flush_interval_ms(0).build();
        assert!(result.is_err());
    }

    #[test]
    fn batch_raft_config_zero_max_batch_size_fails() {
        let result = BatchRaftConfig::builder().max_batch_size(0).build();
        assert!(result.is_err());
    }

    #[test]
    fn batch_raft_config_custom_values() {
        let config = BatchRaftConfig::builder()
            .enabled(true)
            .flush_interval_ms(5)
            .max_batch_size(128)
    ...
    #[test]
    fn batch_raft_config_serde_roundtrip() {
        let config = BatchRaftConfig::default();
        let json = serde_json::to_string(&config).unwrap();
        let deserialized: BatchRaftConfig = serde_json::from_str(&json).unwrap();
        assert_eq!(config, deserialized);
    }

    #[test]
    fn batch_raft_config_serde_defaults_from_empty() {
        let config: BatchRaftConfig = serde_json::from_str("{}").unwrap();
        assert_eq!(config, BatchRaftConfig::default());
    }
}
