...
/// Maximum entries held in `crypto_cache`. Chosen as a safety bound — a healthy
/// cluster reaches terminal state on sagas within seconds, so steady-state
/// occupancy is far below this. Bounds worst-case memory if sagas stall.
const CRYPTO_CACHE_MAX_CAPACITY: u64 = 10_000;

/// Time-to-live for `crypto_cache` entries. Onboarding sagas that have not
/// completed within one hour are already considered abandoned by the rest of
/// the orchestrator (retry backoff caps at 5 min × 10 = 50 min), so stale
/// crypto material can age out without affecting live traffic.
const CRYPTO_CACHE_TTL: Duration = Duration::from_secs(3600);

/// Constructs the bounded, TTL'd cache used for onboarding crypto material.
fn build_crypto_cache() -> Cache<SagaId, OnboardingCrypto> {
    Cache::builder().max_capacity(CRYPTO_CACHE_MAX_CAPACITY).time_to_live(CRYPTO_CACHE_TTL).build()
}
...
pub struct SagaSubmission {
    ...
    /// W3C `traceparent` header captured from the originating gRPC request.
    ///
    /// Stored on the `Saga` record before persistence so every subsequent
    /// poll-cycle step can propagate it into `RegionalProposal` RPCs,
    /// keeping distributed traces linked across the global → regional hop.
    /// Format: `"00-<trace_id>-<span_id>-<flags>"` (see [`TraceContext`]).
...
pub struct SagaOrchestrator<B: StorageBackend + 'static> {
    ...
    ///
    /// Bounded by `CRYPTO_CACHE_MAX_CAPACITY` entries with `CRYPTO_CACHE_TTL`
    /// per-entry expiry so a long-lived leader that accumulates stalled sagas
    /// cannot grow the cache unbounded or hold crypto material forever.
    #[builder(default = build_crypto_cache())]
    crypto_cache: Cache<SagaId, OnboardingCrypto>,
...
impl<B: StorageBackend + 'static> SagaOrchestrator<B> {
    ...
    /// links back to the originating saga trace (W3C Trace Context).
    ...
    ///
    /// `traceparent`, when `Some`, is injected as a W3C `traceparent` gRPC
    /// metadata header on cross-node `RegionalProposal` RPCs.
    ...
    ///
    /// The saga's own `traceparent` is propagated to the underlying regional
    /// proposal so even persistence spans link back to the originating trace.
        ...
        match &saga.state.clone() {
            ...
            DeleteUserSagaState::MarkingDeleted { user_id, remaining_organizations } => {
                ...
                if let Some(organization) = remaining_organizations.first() {
                    ...
                    let traceparent = saga.traceparent.as_deref();

                    // Delete membership in this organization
                    self.delete_entity(*organization, SYSTEM_VAULT_ID, &member_key, traceparent)
                        .await?;

                    // Also delete the index
                    let idx_key = format!("_idx:member:user:{}", user_id.value());
                    let _ = self
                        .delete_entity(*organization, SYSTEM_VAULT_ID, &idx_key, traceparent)
                        .await;
    ...
    async fn delete_entity(
        ...
        self.propose_write(organization, vault, vec![operation], traceparent).await
    }
    ...
    async fn execute_create_user_step(&self, saga: &mut CreateUserSaga) -> Result<(), SagaError> {
        ...
        match saga.state.clone() {
            ...
            CreateUserSagaState::RegionalDataWritten { user_id, user_slug, .. } => {
                ...
                // Propagate the parent saga's trace context to the child so
                // downstream spans link back to the original request.
                .with_traceparent(saga.traceparent.clone());
    ...
    async fn execute_saga(&self, mut saga: Saga) {
        ...
        if saga.is_terminal() {
            ...
            // `Cache::remove` returns `Option<V>` (clones out the value),
            // matching the previous `HashMap::remove` semantics.
            let crypto = self.crypto_cache.remove(&saga_id);
            ...
            if let Some(region) = pii_region {
                self.delete_saga_pii(&saga_id, region, saga.traceparent()).await;
            }
    ...
    ///
    /// For each submission:
    /// 1. Stamps the submission's originating `traceparent` onto the saga record so downstream
    ///    regional proposals can propagate it
    /// 2. Saves the saga record to storage (so it survives crashes)
    /// 3. Persists PII to REGIONAL scratch storage (if present) for step 1 construction
    /// 4. Caches the notification sender (if present) for result delivery
    async fn drain_submissions(&self, rx: &mut mpsc::Receiver<SagaSubmission>) {
        while let Ok(mut submission) = rx.try_recv() {
            // Stamp the originating trace context onto the saga record before
            // persistence so every subsequent step can propagate it.
            if submission.traceparent.is_some() {
                submission.record.set_traceparent(submission.traceparent.clone());
            }
                ...
                for attempt in 0..3u32 {
                    match self
                        .persist_saga_pii(
                            &saga_id,
                            region,
                            &SagaPii::Organization(org_pii.clone()),
                            traceparent.as_deref(),
                        )
                        .await
                    {
    ...
    async fn run_cycle(&self) {
        ...
        // Trace context for the poll cycle itself. Per-saga propagation uses
        // each saga's persisted `traceparent`; this is the cycle-level trace
        // for correlating cycle logs and metrics.
        let trace_ctx = TraceContext::new();

        debug!(trace_id = %trace_ctx.trace_id, "Starting saga poll cycle");
        ...
        if sagas.is_empty() {
            debug!(trace_id = %trace_ctx.trace_id, "No pending sagas");
        } else {
            info!(trace_id = %trace_ctx.trace_id, count = saga_count, "Found pending sagas");
...
mod tests {
    ...
    /// Crypto cache must stay bounded even under sustained insert pressure.
    ///
    /// Regression guard for Finding #31 (audit remediation plan Task 6.2):
    /// the previous `Mutex<HashMap<SagaId, OnboardingCrypto>>` had no upper
    /// bound, so a long-running leader that stalled on sagas could retain
    /// crypto material indefinitely. Replacing it with a `moka::sync::Cache`
    /// caps occupancy at `CRYPTO_CACHE_MAX_CAPACITY`.
    #[test]
    fn crypto_cache_respects_max_capacity() {
        let cache = build_crypto_cache();
        let inserts: u64 = CRYPTO_CACHE_MAX_CAPACITY * 2;

        let sample_crypto = OnboardingCrypto {
            kid: "kid-test".to_string(),
            refresh_token: "rt-test".to_string(),
            refresh_family_id: [0u8; 16],
            refresh_expires_at: chrono::Utc::now(),
            user_slug: UserSlug::new(1),
            organization_slug: OrganizationSlug::new(1),
        };
        ...
        for i in 0..inserts {
            cache.insert(SagaId::new(format!("saga-{i}")), sample_crypto.clone());
        }

        // moka admits entries eagerly then evicts asynchronously on a pending
        // task queue. `run_pending_tasks` forces eviction to complete so the
        // assertion is deterministic.
        cache.run_pending_tasks();

        assert!(
            cache.entry_count() <= CRYPTO_CACHE_MAX_CAPACITY,
            "crypto_cache exceeded max capacity: {} > {}",
            cache.entry_count(),
            CRYPTO_CACHE_MAX_CAPACITY
        );
    }

    /// Crypto cache values survive a round-trip through insert/remove.
    ///
    /// Guards against accidentally breaking the `SagaOutput` construction path
    /// when switching backing stores: the orchestrator relies on
    /// `self.crypto_cache.remove(&saga_id)` returning the `OnboardingCrypto`
    /// that was stashed during step 1.
    #[test]
    fn crypto_cache_insert_remove_round_trip() {
        let cache = build_crypto_cache();
        let saga_id = SagaId::new("round-trip");
        let crypto = OnboardingCrypto {
            kid: "kid-rt".to_string(),
            refresh_token: "rt-rt".to_string(),
            refresh_family_id: [7u8; 16],
            refresh_expires_at: chrono::Utc::now(),
            user_slug: UserSlug::new(42),
            organization_slug: OrganizationSlug::new(99),
        };

        cache.insert(saga_id.clone(), crypto.clone());
        let taken = cache.remove(&saga_id).expect("crypto should be cached");
        assert_eq!(taken.kid, crypto.kid);
        assert_eq!(taken.refresh_token, crypto.refresh_token);
        assert_eq!(taken.user_slug, crypto.user_slug);
        assert_eq!(taken.organization_slug, crypto.organization_slug);

        // Second remove is a miss — value was consumed.
        assert!(cache.remove(&saga_id).is_none());
    }
    ...
    #[test]
    fn saga_submission_traceparent_stamps_persisted_record() {
        let tp = "00-0af7651916cd43dd8448eb211c80319c-b7ad6b7169203331-01".to_string();
        let mut record = Saga::DeleteUser(DeleteUserSaga::new(
            SagaId::new("stamp-test"),
            DeleteUserInput { user: UserId::new(7), organization_ids: vec![] },
        ));
        // Initially, the record carries no trace context.
        assert_eq!(record.traceparent(), None);

        // Simulate the `drain_submissions` stamping step.
        record.set_traceparent(Some(tp.clone()));
        assert_eq!(record.traceparent(), Some(tp.as_str()));

        // Round-trips through the `_meta:saga:` JSON encoding used for
        // persistence — so a leader crash + reload preserves the trace.
        let encoded = serde_json::to_vec(&record).expect("saga serializes");
        let decoded: Saga = serde_json::from_slice(&encoded).expect("saga deserializes");
        assert_eq!(decoded.traceparent(), Some(tp.as_str()));
    }
    ...
    fn persisted_saga_traceparent_matches_origin_trace_id() {
        ...
        let origin_trace_id = "0af7651916cd43dd8448eb211c80319c";
        let tp = format!("00-{origin_trace_id}-b7ad6b7169203331-01");

        let saga = Saga::CreateOnboardingUser(
            inferadb_ledger_state::system::CreateOnboardingUserSaga::new(
                SagaId::new("trace-origin"),
                inferadb_ledger_state::system::CreateOnboardingUserInput {
                    email_hmac: "hmac".into(),
                    region: Region::US_EAST_VA,
                    user_slug: UserSlug::new(1),
                    organization_slug: OrganizationSlug::new(2),
                },
            )
