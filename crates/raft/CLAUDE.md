...
- **Handler-phase event batching** (Sprint 1B4 + Sprint 1B5 Fix #2): `EventHandle::record_handler_event` enqueues into a bounded `FlushQueue`; a single `EventFlusher` task per `EventHandle` drains on time / size / shutdown trigger and commits via `commit_in_memory` (Sprint 1B5 flipped this from `commit()` to `commit_in_memory()` — durability is realised by the `StateCheckpointer`'s per-tick events.db sync, same contract as apply-phase events). `EventHandle::flush_for_shutdown` is invoked from `GracefulShutdown` Phase 5b between WAL flush (5a) and `sync_all_state_dbs` (5c); Phase 5c is what fsyncs the drained batches before exit. `EventWriterBatchConfig::enabled = false` bypasses the queue entirely, routes emissions through `EventWriter::write_entry` directly, and restores pre-1B4 per-emission fsync semantics — see [`event_writer.rs`](src/event_writer.rs) and `docs/operations/durability.md` § "Handler-phase event flush window".
