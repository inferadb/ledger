...
- **Handler-phase event batching** (Sprint 1B4): `EventHandle::record_handler_event` enqueues into a bounded `FlushQueue`; a single `EventFlusher` task per `EventHandle` drains on time / size / shutdown trigger and commits durably via `EventWriter::write_entry` (one fsync per batch). `EventHandle::flush_for_shutdown` is invoked from `GracefulShutdown` Phase 5b between WAL flush (5a) and `sync_all_state_dbs` (5c). `EventWriterBatchConfig::enabled = false` bypasses the queue and restores pre-1B4 per-emission fsync semantics — see [`event_writer.rs`](src/event_writer.rs) and `docs/operations/durability.md` § "Handler-phase event flush window".
