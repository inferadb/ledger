...
14. **Per-organization `OrganizationGroup`s default to `LeadershipMode::Delegated`.** Election timeouts on their `ConsensusState`s are disabled; the leader is whoever leads the parent `RegionGroup`. A watcher task (`start_organization_group` in `raft_manager.rs`) propagates the region leader into each per-org shard via `ConsensusEngine::adopt_leader` → `ConsensusState::adopt_leader`. Per-organization self-election (`LeadershipMode::SelfElect`) is a future runtime escape hatch; the data plane (replication, apply, state) is mode-agnostic, so flipping the mode does not require changing the apply path or the state machine.
