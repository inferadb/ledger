...
13. **Cross-tier communication flows only through typed channels.** `OrganizationGroup` never proposes coordination state; `RegionGroup` never proposes data; `SystemGroup` never proposes regional or per-organization state. The three group types share no apply path and no persistence — they communicate exclusively through the typed `CoordinationEvent` / `OrganizationEvent` / `RegionLeaderState` channels (e.g., `organization_creation_rx` on `RaftManager`, the `Delegated` leadership watcher on each per-org `Shard`). Bypassing these channels (region apply directly mutating organization Raft state, or vice versa) is a tier violation and breaks root rule 15. Audited by `consensus-reviewer`.

14. **Per-organization `OrganizationGroup`s default to `LeadershipMode::Delegated`.** Election timeouts on their `Shard`s are disabled; the leader is whoever leads the parent `RegionGroup`. A watcher task (`start_organization_group` in `raft_manager.rs`) propagates the region leader into each per-org shard via `ConsensusEngine::adopt_leader` → `Shard::adopt_leader`. Per-organization self-election (`LeadershipMode::SelfElect`) is a future runtime escape hatch; the data plane (replication, apply, state) is mode-agnostic, so flipping the mode does not require changing the apply path or the state machine.
