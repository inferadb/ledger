...
In multi-region deployments, the `MultiRegionWriteService` transparently forwards writes to the correct region's leader. Clients can send write requests to **any node** in the cluster -- if the target organization lives in a different region, the receiving node forwards the request to that region's leader via `RegionResolver`.
...
Clients do not need region-aware routing for writes. Any node accepts writes for any organization. The same forwarding applies for reads via `MultiRegionReadService`.
