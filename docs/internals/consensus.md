...
Pre-flight checks (validation, rate limiting, quota) execute on the originating node before forwarding. Vault slug resolution and idempotency deduplication happen on the destination leader. The `WriteService` uses `resolve_with_forward()` to determine whether to handle locally or forward.
